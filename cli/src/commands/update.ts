...
async function getPackageVersion(): Promise<string> {
  ...
  const packagePath =
    packageCandidates.find((path) => existsSync(path)) ?? packageCandidates[0];
  ...
  try {
    ...
    if (currentVersion !== latestVersion) {
      ...
      try {
        ...
        console.log();
        logger.error(
          "Automatic update failed (you may need elevated/admin permissions).",
        );
        logger.info(
      ...
      logger.info(
        `Now rerun ${chalk.cyan(`uipro init${aiFlag} --force${globalFlag}`)} to refresh your skill files.`,
      );
      return;
    }
