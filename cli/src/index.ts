...
program
  ...
  .option(
    "-o, --offline",
    "Compatibility flag; template installs use bundled assets",
  )
  .option(
    "-g, --global",
    "Install globally to home directory (~/) instead of current project",
  )
  .option(
    "-t, --token <token>",
...
program
  ...
  .option(
    "-g, --global",
    "Uninstall from home directory (~/) instead of current project",
  )
  .action(async (options) => {
    if (options.ai && !AI_TYPES.includes(options.ai)) {
      console.error(`Invalid AI type: ${options.ai}`);
