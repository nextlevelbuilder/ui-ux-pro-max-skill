...
function row(status: string, id: string, replacement = ""): string {
  const values: Record<string, string> = {
    Status: status,
    "Style Category": id,
    No: status === "active" ? "1" : "2",
    "Style ID": id,
    Aliases: `${id}-alias`,
    "Replacement Domain": replacement,
    "Replacement ID": replacement ? "Hero + CTA" : "",
    Type: "General",
    Keywords: "clean",
    "Primary Colors": "#ffffff",
    "Secondary Colors": "#000000",
    "CSS/Technical Keywords": "display: grid",
    Complexity: "Low",
  };
  return headers
    .map((header) => JSON.stringify(values[header] || ""))
    .join(",");
}

test("parses taxonomy fields by header name", () => {
  const styles = parseStylesCSV(
    [headers.join(","), row("deprecated", "legacy-layout", "landing")].join(
      "\n",
    ),
  );
...
test("gallery defaults to active and exposes explicit status views", () => {
  ...
  assert.deepEqual(
    filterStyles(styles, DEFAULT_FILTERS).map((s) => s.styleId),
    ["active-style"],
  );
  assert.deepEqual(
  ...
  assert.equal(
    filterStyles(styles, { ...DEFAULT_FILTERS, status: "" }).length,
    3,
  );
});
...
test("parser rejects missing or unknown status", () => {
  ...
  assert.throws(
    () =>
      parseStylesCSV(
        [headers.join(","), row("draft", "draft-style")].join("\n"),
      ),
    /Unknown or missing style status/,
  );
});
